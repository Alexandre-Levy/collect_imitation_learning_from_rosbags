...
def get_waypoints_by_quadratic_interpollation(timestamp_gps, timestamp_img, x, y, theta):
    ...
    for i in range(0, num_interpolation, nb_pt):  #num_interpolation
        ...
        if isinstance(pts_hist,np.ndarray):
            if pts_hist.any():
                ...
                if first_legend == False:
                    plt.plot(pts_hist[:,0], pts_hist[:,1],'x', markersize=4, color ="blue", label="waypoints from quadratic interpolation")
                    first_legend = True
                else:
    ...
    return x_img_positions, y_img_positions, theta_img_positions
...
def main(args=None):
    ...
    x_img_positions, y_img_positions, theta_img_positions = get_waypoints_by_quadratic_interpollation(timestamp_gps, timestamp_img, x, y, theta)
    ...
    plt.plot(x, y, 'o', markersize=4, color ="green", label="rtk positions")

    plt.plot(waypoints[:,0], waypoints[:,1],'o', markersize=3, color ="red", label="waypoints from linear interpolation")
    plt.title("RTK and waypoints")
    plt.legend(loc='lower right')
    plt.get_current_fig_manager().canvas.set_window_title("RTK and waypoints")
    plt.show()
